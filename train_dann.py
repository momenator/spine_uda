...
from models.modules import NLayerDiscriminator, ReverseLayerF
...
train_scan_dataset = UnpairedDataset('../', 'ct_sag_kr/train', 'mr_sag_kr/train_reduced')
...
domain_net = NLayerDiscriminator(512, n_layers=2)
domain_net.cuda()
...
d_optimiser = optim.SGD(domain_net.parameters(), lr=learning_rate)
...
for e in range(epochs):
    ...
    for i, data in enumerate(tqdm.tqdm(train_loader)):
        ...
        domain_b_result = domain_net(reverse_feature_b)
        b_label = torch.ones(domain_b_result.size(), requires_grad=False).cuda()
        
        domain_b_loss = nn.BCELoss()(domain_b_result, b_label)
        
        loss = loss_seg_a + domain_a_loss + domain_b_loss
